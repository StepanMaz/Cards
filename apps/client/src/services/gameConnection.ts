import type { ActionCreatorWithPayload, Middleware, PayloadAction } from "@reduxjs/toolkit";
...
export class GameConnection {
    ...
    createMiddleware = <T extends Record<string, any>>(
        config: SocketMiddlewareConfig<T, ServerEvents, ClientEvents>
    ): Middleware =>
        (store) => {
            const { on = {}, emit = {} } = config;

            const emitMap: Map<string, string> = new Map(
                Object.entries(emit)
            );
...
export type GetFromArray<T> =
    T extends [infer First, ...infer Rest] ?
    (First extends ActionCreatorWithPayload<infer Type, infer Name> ? { [P in Name]: Type } : never) &
    GetFromArray<Rest> : {}

type KeysTypeBinding<T1 extends Record<string, any>, T2 extends Record<string, any>> = {
    [K1 in keyof T1]?: {
        [K2 in keyof T2]: T1[K1] extends T2[K2] ? K2 : never
    }[keyof T2] | String
}

export type GetFromObj<T extends Record<string, any>> = UnionToIntersection<Extract<{
    [K1 in {
        [K in keyof T]: T[K] extends ActionCreatorWithPayload<any, string> ? K : never
    }[keyof T]]: T[K1]
}>>

type Extract<T extends Record<string, ActionCreatorWithPayload<any, string>>> = {
    [K in keyof T]: T[K] extends ActionCreatorWithPayload<infer Type, infer Name> ? Record<Name, Type> : never
}[keyof T]
