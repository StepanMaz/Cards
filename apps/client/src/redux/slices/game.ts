...
const gameSlice = createSlice({
    ...
    reducers: {
        ...
        removePlayer(state, action: PayloadAction<{ id: string }>) {
            const index = state.players.indexOf(action.payload.id);
            if (index >= 0) state.players.splice(index);
        },
        ...
        setPlayerHand(
            state,
            action: PayloadAction<{ cards: Card<Suit, string | number>[] }>,
        ) {
            state.hand = action.payload.cards;
        },
        removeFromHand(
            state,
            action: PayloadAction<{ card: Card<Suit, string | number> }>,
        ) {
        ...
        swapHand(state, action: PayloadAction<{ from: number; to: number }>) {
            ...
            if (from > to) {
                state.hand = state.hand
                    .slice(0, to)
                    .concat(element)
                    .concat(...state.hand.slice(to, from))
                    .concat(...state.hand.slice(from + 1));
            }
            /*else if (to > from) {
