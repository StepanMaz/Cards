...
export class TrumpCardDeckBuilder<TSuit, TValue> {
    ...
    static policies = {
        last: <TSuit, TValue>(stack: readonly Card<TSuit, TValue>[]) => stack.at(-1),
        const: <TSuit, TValue>(trump_card: Card<TSuit, TValue | undefined>) => (stack: readonly Card<TSuit, TValue>[]) => trump_card
    } as const
}